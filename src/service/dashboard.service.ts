...
import { db } from "../db";
import { payment, santri, SANTRI_STATUS, STATUS_PAYMENT, TYPE_PAYMENT } from "../models";
...
const dashboardService = {
	async getDashboardSummaryAdmin() {
		...
		const [{ sum: totalRegistrasi }] = await db
			...
			.from(payment)
			.where(and(eq(payment.type, TYPE_PAYMENT.REGISTRATION), eq(payment.status, STATUS_PAYMENT.COMPLETED)));

		const [{ sum: totalSPP }] = await db
			.select({ sum: sum(payment.amount).as("sum") })
			.from(payment)
			.where(and(eq(payment.type, TYPE_PAYMENT.SPP), eq(payment.status, STATUS_PAYMENT.COMPLETED)));
